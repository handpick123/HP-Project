...
def created_data():
        ...
        process_syntax=syntaxs_df.copy() #[~syntaxs_df["Step"].str.contains('HP', na=False)]
        # process_syntax
        ...
        order_df_f=order_df_.drop(columns={'STEP'})
        ...
        return new_status,order_df_f,order_tm,progress
...
order_pb=order_pb[['ID_ORDER','TÊN_TTF','S/L','SƠN']]
order_pb['Check']=order_pb['ID_ORDER'] +" _ "+order_pb["TÊN_TTF"]
...
#     color1 = 'orange' if val=='0. Chưa cập nhật' else 'yellow' if 'sai' in str(val) or "chưa" in str(val) else 'red' if val=='Hủy đơn hàng' else 'white'
#     color2 = 'blue' if val=='' else 'white'
...
#     c2 = '' 
#     #compare columns
#     mask = x['Ngày_giải_quyết'] != "2"
#     #DataFrame with same index and columns names as original filled empty strings
#     df1 =  pd.DataFrame(c2, index=x.index, columns=x.columns)
#     #modify values of df1 column by boolean mask
#     df1.loc[mask, 'Ngày_giải_quyết'] = c1
#     return df1
...
#             bp_df_=bp_df[['ID_ORDER','NGÀY_XUẤT','TÊN_TTF','S/L','SƠN','Tình_trạng','Ngày_giải_quyết']].reset_index(drop=True)

#             st.markdown("#### 0. Chưa cập nhật: {} mã - {} sp".format(len(bp_df_['ID_ORDER'].tolist()),sum(bp_df_['S/L'].astype(int)),unsafe_allow_html=True))

#             st.dataframe(bp_df_.style.applymap(color_survived, subset=['Tình_trạng','Ngày_giải_quyết']),height =200,width=850)
#             st.markdown('')
#             st. markdown('')     
...
#                 st.dataframe(bp_df_.style.applymap(select_col),height =200,width=850)
#                 st.markdown('')
#                 st.markdown('')     
...
#                 st.dataframe(bp_df_.style.applymap(select_col),height =200,width=300)
